...
class MetabaseApi:
    ...
    def query (self, method, query_name, json_data = None):
        ...
        try:
            ...
            if (r.text):
                ...
                error_msg = f"HTTP {r.status_code}: {r.text[:1000]}"
                if self.debug:
                    print(f"Full response text: {r.text}")
    ...
    def card_id2name(self, database_name, card_id):
        if self.dashcards_export is None:
            self.dashcards_export = self.get_dashcards(database_name)
        for card in self.dashcards_export:
            if card['id'] == card_id:
                return card['name']
        return ''
    ...
    def get_dashcards(self, database_name):
        database_id = self.database_name2id(database_name)
    ...
    def card_name2id(self, database_name, card_name):
        if not self.dashcards_name2id:
            for c in self.get_dashcards(database_name):
                self.dashcards_name2id[c['name']] = c['id']
        return self.dashcards_name2id.get(card_name)
    ...
    def export_dashcards_to_json(self, database_name, dirname):
        export = self.get_dashcards(database_name)
        for card in export:
    ...
    def dashboard_import_with_cards(self, database_name, dash_from_json, prepared_cards=None):
        """Import dashboard with tabs and cards in a single PUT request (Metabase 0.48.0+)"""
        dashid = self.dashboard_name2id(database_name, dash_from_json['name'])
        
        # Create dashboard data with only writable fields
        dashboard_data = {}
        
        # If adding cards, send ONLY the dashcards field to avoid conflicts
        if prepared_cards is not None:
            dashboard_data['dashcards'] = prepared_cards
        else:
            # Include only the fields that can be written
            writable_fields = [
                'name', 'description', 'caveats', 
                'cache_ttl', 'embedding_params', 'enable_embedding',
                'collection_id', 'collection_position', 'archived',
                'auto_apply_filters', 'width', 'position', 'show_in_getting_started'
            ]
            
            for field in writable_fields:
                if field in dash_from_json:
                    dashboard_data[field] = dash_from_json[field]
            
            # Clean up parameters - remove sectionId which is read-only
            if 'parameters' in dash_from_json and dash_from_json['parameters']:
                cleaned_params = []
                for param in dash_from_json['parameters']:
                    param_copy = param.copy()
                    # Remove read-only fields
                    if 'sectionId' in param_copy:
                        del param_copy['sectionId']
                    cleaned_params.append(param_copy)
                dashboard_data['parameters'] = cleaned_params
            
            # Prepare tabs with negative IDs for new tabs (Metabase 0.48.0+)
            if 'tabs' in dash_from_json and dash_from_json['tabs'] and len(dash_from_json['tabs']) > 0:
                tabs_to_import = []
                for idx, tab in enumerate(dash_from_json['tabs']):
                    tab_copy = tab.copy()
                    # Remove fields that shouldn't be sent
                    for field in ['id', 'created_at', 'updated_at', 'entity_id', 'dashboard_name', 'dashboard_id']:
                        if field in tab_copy:
                            del tab_copy[field]
                    # Assign negative ID for new tab
                    tab_copy['id'] = -(idx + 1)
                    tabs_to_import.append(tab_copy)
                dashboard_data['tabs'] = tabs_to_import
            elif 'tabs' in dash_from_json and (not dash_from_json['tabs'] or len(dash_from_json['tabs']) == 0):
                # Explicitly include empty tabs array for dashboards without tabs (only on first request)
                dashboard_data['tabs'] = []
        
        if dashid:
            result = self.query('PUT', 'dashboard/'+str(dashid), dashboard_data)
        else:
            self.dashboards_name2id = None
            result = self.query('POST', 'dashboard', dashboard_data)
        
        return result
    
    def get_tab_id_mapping(self, database_name, dashboard_name, original_tabs):
        """Get mapping of old tab IDs to new tab IDs by comparing tab names"""
        dashboard = self.get_dashboard(database_name, dashboard_name)
        new_tabs = dashboard.get('tabs', [])
        
        tab_id_mapping = {}
        
        # Create mapping by matching tab names
        for old_tab in original_tabs:
            old_id = old_tab.get('id')
            old_name = old_tab.get('name')
            
            for new_tab in new_tabs:
                if new_tab.get('name') == old_name:
                    tab_id_mapping[old_id] = new_tab['id']
                    break
        
        return tab_id_mapping
    ...
    def card_import(self, database_name, card_from_json):
        ...
        if 'parameters' in card_from_json and card_from_json['parameters']:
            for param in card_from_json['parameters']:
                if param.get('values_source_config') and param['values_source_config'].get('card_id') is None:
    ...
    def metric_import(self, database_name, metric_from_json):
        ...
        if 'parameters' in metric_from_json and metric_from_json['parameters']:
            for param in metric_from_json['parameters']:
                if param.get('values_source_config') and param['values_source_config'].get('card_id') is None:
                    del param['values_source_config']
                    if 'values_source_type' in param:
                        del param['values_source_type']
    ...
    def prepare_dashboard_cards(self, cards_list, tab_id_mapping=None):
        """Prepare dashboard cards for import (Metabase v0.48.0+)"""
        
        # Prepare cards for import
        bulk_cards = []
        for idx, ordered_card in enumerate(cards_list):
            # Only include essential fields for dashboard cards
            card_data = {
            ...
            # Check if this is a virtual card (text/heading)
            vis_settings = ordered_card.get('visualization_settings', {})
            is_virtual_card = vis_settings.get('virtual_card') is not None
            
            # Add card_id if present (but not for virtual cards)
            if ordered_card.get('card_id') and not is_virtual_card:
                card_data['card_id'] = ordered_card['card_id']
            
            # For virtual cards, use an empty card object (the virtual_card is in visualization_settings)
            if is_virtual_card:
                card_data['card'] = {}
            elif not ordered_card.get('card_id') and ordered_card.get('card'):
                # For non-virtual cards without card_id, include the card object
                card_data['card'] = ordered_card['card']
            
            # Add optional fields if present and not empty
            if 'visualization_settings' in ordered_card and ordered_card['visualization_settings']:
                vis_settings_copy = copy.deepcopy(ordered_card['visualization_settings'])
                # For virtual cards, clean up read-only fields from virtual_card
                if is_virtual_card and 'virtual_card' in vis_settings_copy:
                    virtual_card = vis_settings_copy['virtual_card']
                    # Remove read-only fields that might cause issues
                    for field in ['archived', 'name']:
                        if field in virtual_card:
                            del virtual_card[field]
                card_data['visualization_settings'] = vis_settings_copy
            
            if 'parameter_mappings' in ordered_card and ordered_card['parameter_mappings']:
                # Deep copy and update card_id references in parameter mappings
                parameter_mappings = copy.deepcopy(ordered_card['parameter_mappings'])
                if card_data.get('card_id'):
                    # Update card_id in parameter mappings to use new card ID
                    for mapping in parameter_mappings:
                        if 'card_id' in mapping:
                            mapping['card_id'] = card_data['card_id']
                card_data['parameter_mappings'] = parameter_mappings
            
            if 'action_id' in ordered_card and ordered_card['action_id']:
                card_data['action_id'] = ordered_card['action_id']
            
            # Map old tab ID to new tab ID (only include if mapping exists)
            if 'dashboard_tab_id' in ordered_card and ordered_card['dashboard_tab_id'] and tab_id_mapping:
                old_tab_id = ordered_card['dashboard_tab_id']
                if old_tab_id in tab_id_mapping:
                    card_data['dashboard_tab_id'] = tab_id_mapping[old_tab_id]
                # If old_tab_id not in mapping, don't include dashboard_tab_id at all
            
            bulk_cards.append(card_data)
    ...
    def import_dashcards_from_json(self, database_name, dirname, collection_name = None):
        res = []
    ...
    def import_dashboards_from_json(self, database_name, dirname, collection_name = None):
        ...
        if len(jsondata):
            ...
            for dash in jsondata:
                dashboard_name = dash.get('name', 'Unknown')
                print(f"\nImporting dashboard: {dashboard_name}")
                
                # Store original tabs and cards
                original_tabs = dash.get('tabs', [])
                original_cards = dash.get('dashcards', [])
                
                # First, import dashboard with tabs (without cards)
                res[0].append(self.dashboard_import_with_cards(database_name, dash, None))
                print(f"  ✓ Dashboard structure with tabs imported")
                
                # Get tab ID mapping by comparing names
                tab_id_mapping = None
                if original_tabs and len(original_tabs) > 0:
                    tab_id_mapping = self.get_tab_id_mapping(database_name, dash['name'], original_tabs)
                    res[1].append(tab_id_mapping)
                    print(f"  ✓ Tab mapping created: {tab_id_mapping}")
                
                # Prepare and import cards with tab mapping
                if original_cards:
                    print(f"  → Importing {len(original_cards)} cards...")
                    prepared_cards = self.prepare_dashboard_cards(original_cards, tab_id_mapping)
                    res[2].append(self.dashboard_import_with_cards(database_name, dash, prepared_cards))
                    print(f"  ✓ Cards imported")
        return res
